...
export default function Card({
  ...
  return (
    <div
      className={`flex flex-col gap-2 px-2 md:px-2 py-2 md:py-4 overflow-x-hidden text-sm border rounded w-96 ${
    ...
    >
        ...
        <div className='flex items-end justify-between gap-2  w-full'>
          <div className='flex flex-col w-full'>
            ...
            <div className='flex justify-between gap-2'>
              ...
              <div>
                <span className='opacity-60 whitespace-nowrap text-xs md:text-sm'>
                  Lasts for
                </span>{' '}
                <span className='whitespace-nowrap text-xs md:text-sm'>{`${Math.floor(
                  duration / 3600
                )}hr ${Math.floor((duration % 3600) / 60)}mins `}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className='flex items-center justify-between md:gap-2 text-xxs'>
        <span>
          <span className='opacity-60'>Starts on:</span> {useTimeString(start)}
        </span>
        <span>
      ...
      <div className='flex items-center justify-between gap-2 mt-2'>
        <a
          ...
          href={`http://www.google.com/calendar/event?action=TEMPLATE&dates=${start?.replace(
            /-|:|\.\d\d\d/g,
            ''
          )}%2F${end?.replace(/-|:|\.\d\d\d/g, '')}&text=${encodeURIComponent(
            event || ''
