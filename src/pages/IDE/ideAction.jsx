...
import timeBeen from '../../../utils/timeBeen';
...
export default function IdeAction({
  ...
  const [info, setInfo] = React.useState(
    `Last Saved ${timeBeen(
      Math.floor((new Date() - lastSavedTime) / 1000)
    )} ago` || ''
  );
  ...
  return (
    <div
      style={{
        paddingTop: isFullScreen || selectOptionsHidden ? '10px' : '4px',
      }}
      className='flex items-center justify-end px-12 pb-1 -my-0.5 bg-dark'
    >
      {author && snippetTitle && (
